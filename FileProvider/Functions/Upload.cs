...
public class Upload(ILogger<Upload> logger, FileService fileService)
{
    ...
    public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Function, "post")] HttpRequest req)
    {
        try
        {
            if (req.Form.Files["file"] is IFormFile file) //hämtar ut olika delar av formuläret
            {
                ...
                await _fileService.SetBlobContainerAsync(fileEntity.ContainerName); //skapas en conatiner baserat på namnet
